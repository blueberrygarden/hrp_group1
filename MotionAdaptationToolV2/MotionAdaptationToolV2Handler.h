...
class MotionAdaptationToolV2Handler : public MotionHandler
{
    Q_OBJECT

public:
    MotionAdaptationToolV2Handler(QWidget* widget);
    ...
    virtual void motionsOpened(bool opened, MMM::MotionRecordingPtr motions, std::map<std::string, VirtualRobot::RobotPtr> /*currentRobots*/) {
        if (dialog && dialog->isVisible() && opened) dialog->open(motions);
    }

    static constexpr const char* NAME = "MotionAdaptationToolV2Handler";

private:
    std::string searchPath;
    QWidget* widget;
    MotionAdaptationToolV2HandlerDialog* dialog;
};
