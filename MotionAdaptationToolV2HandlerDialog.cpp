...
void MotionAdaptationToolV2HandlerDialog::jumpTo(float timestep) {
    ...
    if (motion) {
        ...
        auto robotNodeSet = mmm_visualized->getRobotNodeSet("BodyRightArm"); // retrieves the robot node set containing joint nodes of both arms and both legs as set in the mmm.xml as well as virtual joints to change the rootpose
        ...
        for (const std::pair<std::string, std::map<float, std::map<std::string, float>>> &nodeDistances : distancesByNode) {
            ...
            cik.addTarget(mmm_visualized->getRobotNode(nodeName), mmm_visualized->toLocalCoordinateSystem(pose), VirtualRobot::IKSolver::CartesianSelection::Position);
        }
        ...
        for (const std::string &nodeName : robotNodes) {
            ...
            for (float timestep : mmm_motion->getTimesteps()) {
                ...
                for (auto object : objects) {
                    ...
                    Eigen::Vector3f globalObjectPosition = globalObjectPose.block(0, 3, 3, 1);
                    ...
                    distances[timestep][object.first] = distance;
                }
